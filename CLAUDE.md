...
**这条规则覆盖所有其他规则。** 不论在哪个阶段，AI **绝不**直接 Edit / Write 下列文件：

| 文件 | 谁能写 | AI 该做什么 |
|---|---|---|
| `METHOD.md` | 🚫 **仅用户** | 在 chat 里起草"准备粘贴"的修订段，等用户手动应用 |
| `FINDINGS.md` | 🚫 **仅用户** | 在 chat 里起草新 finding 条目，等用户手动追加 |

其他文件（`STATUS.md` / `experiments/.../*` / `src/` / `archive/` / `templates/` 等）AI 按既有规则写。

**Why**: METHOD 与 FINDINGS 是"研究合同"和"科学主张"。AI 重新措辞会引入隐性语义漂移；用户必须保有完全主权。
...
| `[revising]` | 用户给出反馈后，**起草** METHOD.md 修订建议（不直接编辑），等用户手动应用 |
...
→ **指标硬门槛**：进入 `[executing]` 前 plan.md 的"成功判据"必须显式回答：
  - **本任务能否量化衡量？** Yes / No
  - **能** → 必须给出具体指标 + 通过 / 失败阈值（不是 "效果好" 这种话），实验跑完必须计算这些指标
  - **不能** → 必须写出**为什么不能**，并指定改用什么定性判据（人工评估、可视化对比等）
  - ⚠️ "我不知道怎么衡量" **不是**有效的 No —— 那是"还没想清楚"，打回 `[planning]`
...
### 当用户反馈"需要调整方案"
→ 进入 `[revising]`。
→ **AI 不直接编辑 METHOD.md**。在 chat 里给出"准备粘贴"的修订草稿，包含：
  - 改动了什么
  - 改动的原因（关联到具体的 `experiments/.../REPORT.md`）
  - 新的技术路线
  - 建议的版本号（v(N+1)）
→ 用户手动把草稿粘贴到 `METHOD.md` 的"修订历史"段（**追加**，不要删旧的）
→ 用户确认已应用后，AI 更新 `STATUS.md` 的 `METHOD版本` 字段
→ 回到 `[planning]` 准备下一轮

### 当一次实验产出"跨实验级别的发现"
判断标准：发现不只属于本次实验，而是揭示了**更普遍的规律**（例："输入长度超 2k 时所有方法都退化"、"温度 > 0.7 后所有变体趋同"）。
→ **AI 不直接编辑 FINDINGS.md**。在 chat 里给出"准备粘贴"的条目草稿（必须含 ≥1 个 REPORT.md 引用作为证据）。
→ 询问用户："这看起来是跨实验规律。建议条目如下，确认后请你手动追加：……"
→ 用户手动追加到 `FINDINGS.md`（append-only）后，回到原阶段。
...
仅在 `[executing]` 阶段适用。其他阶段按各自规则走。

### 表面假设，不要静默选择
- 多种合理解读时，**列出来让用户选**，不要默默挑一个
- 任何 "我假设你的意思是..." 必须 surface 给用户确认
- 拿不准就停，别边猜边写

### 写代码前先写"做完的判据"（红→绿循环）

**尽可能要求所有改动**都先写一个最小验证，跑出红 → 改 → 跑出绿。

| 改动类型 | 写什么验证 |
|---|---|
| 修 bug | 复现 bug 的 test / assert / sanity 脚本 |
| 新功能 / 进 `src/` 的代码 | "输入 X 应产生 Y" 的 assert 或单元测试 |
| 重构 / 优化 | 前后 bit-equality 或输出 hash 对比 |
| 实验主脚本（会复用 / 迭代） | 至少 "运行不报错 + 输出形状合理" 的 sanity 检查 |
| **一次性脚本 / notebook** | **可豁免**，但必须在脚本顶部注明 `# one-shot, sanity exempt: <原因>` |

要点：
- "test" ≠ pytest —— 一行 assert / toy data 跑通 / 输出 hash 都算
- 跳过红→绿等于没改 —— 你不知道是否真改对，也没有回归保护
- 豁免必须**显式写出**"一次性"理由；不要把所有东西都说成一次性来逃避

### 最小改动 (Surgical)
- 只改与当前任务直接相关的代码
- **不要"顺手"** 改邻近代码 / 注释 / 格式
- 匹配现有风格，即使你觉得不好
- 注意到无关 dead code 只**指出**，不删除（除非用户授权）
- 你的改动产生的 orphan import / 未用变量 —— 你负责清掉
...
**第一层 — 写代码时**：
- 不写未被要求的功能 / 配置项 / 抽象层
- "未来可能有用" 不是写代码的理由 —— 真有需求了再加
- 偏好"直白的 5 行" 而不是"聪明的 2 行"

**第二层 — 写完代码后强制做"简化扫描"，在 chat 报告结果**：

逐行 / 逐抽象问自己：
- 这一行删了会怎样？无副作用就删
- 这个抽象 / class / helper 在 >1 处用了吗？只有 1 处就内联
- 这个 config / flag 是用户要求的吗？不是就删
- 这个 error handling 覆盖的场景真会发生吗？不会就删
- 这个 type hint / 防御性检查在动态类型上下文里有意义吗？没有就删

报告格式（在 chat 里写）：
> 简化扫描：原 N 行 → 简化后 M 行。删除项：xxx, yyy。
>
> 或：简化扫描：保持 N 行不变，因为 ...

如果代码 > 50 行还没做减法，必须在 chat 里解释每个抽象的存在理由。

**跳过简化扫描等于交付未完成的代码。**

### 适用边界
研究代码经常**应当**被推翻。本节规则约束"未授权时不擅自改"，不阻止用户批准的重构。
...
- ❌ 跳过"先写验证"直接动手改代码（修 bug / 新功能 / 重构都要先写红→绿验证；只有一次性脚本可豁免且需注明）
- ❌ 研究任务上用"探索性所以没法衡量"绕开指标 —— 真探索就明确写 "no metric, because..."；"不知道怎么衡量"等于还没想清楚
- ❌ 顺手改邻近代码 / 注释 / 格式 —— 与本次任务无关的改动会污染 diff
- ❌ 直接 Edit/Write `METHOD.md` 或 `FINDINGS.md`（哪怕在 `[revising]` 阶段）—— 只能起草草稿，由用户手动应用
- ❌ 写完代码不做"简化扫描"就交付
- ❌ 偏好"聪明的 2 行" 而不是"直白的 5 行" —— 研究代码读者通常是未来的你或合作者，简单是给他们的礼物
